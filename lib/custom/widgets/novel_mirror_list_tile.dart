...
class NovelMirrorListTile extends StatefulWidget {
  final int id;
  final NovelStore novelStore;

  const NovelMirrorListTile({
    Key? key,
    required this.id,
    required this.novelStore,
  }) : super(key: key);

  @override
  State<NovelMirrorListTile> createState() => _NovelMirrorListTileState();
}

class _NovelMirrorListTileState extends State<NovelMirrorListTile> {
  bool? _isMirrored;
  ...
  Future<void> _checkStatus() async {
    ...
    try {
      final res = await SyncService.getNovelMirrorStatus(widget.id);
      if (mounted) {
        setState(() {
      ...
      onTap: isMirrored == null
          ...
          : () async {
              if (isMirrored == true) {
                ...
                try {
                  final success =
                      await NovelMirrorService.fetchFromMirror(
                          widget.novelStore);
                  if (success) {
                    BotToast.showText(text: '已从镜像加载');
                  } else {
                ...
                final cancel = BotToast.showLoading();
                try {
                  final result = await SyncService.mirrorNovel(widget.id);
                  if (result != null) {
