...
4. 提供“收藏导出与失效追踪”功能，由服务端每日拉取 Pixiv 用户全部公开收藏，增量写入数据库并标记从 Pixiv 收藏夹中消失的插画。
...
收藏导出功能由服务端后台 worker 完成，目标是解决 Pixiv 在插画失效、作者删除或不可见时可能直接从用户收藏夹移除且不通知用户的问题。后端每日对已保存 Pixiv 凭证的用户执行一次完整公开收藏导出：

1. 使用 Pixiv 官方 `GET /v1/user/bookmarks/illust?user_id={pixiv_user_id}&restrict=public` 作为首个请求。
2. 按响应中的 `next_url` 逐页拉取，直到 `next_url` 为空。
3. 每个收藏插画解析为服务端 Pixiv 响应模型，并将完整插画 JSON 写入数据库。
4. 写入策略必须是增量 upsert，禁止“先删后查”。这样即使某个插画已从 Pixiv 收藏页消失，历史记录仍保留。
5. 若本轮返回的插画 ID 已存在且未标记失效，直接跳过数据库更新，不刷新 JSON、标题、统计数等字段，避免全量导出产生大量无意义更新。
6. 若本轮返回的收藏插画图片 URL 包含 `limit_unknown_360` 占位图，将该插画标记为 `removed=true`，并且不把它视为本轮有效出现。
7. 单次完整导出成功结束后，将同一用户同一 `restrict` 下“本轮未出现但历史存在且尚未移除”的记录标记为 `removed=true`，并写入 `removed_at`。
8. 若导出中途失败，不执行缺失标记，避免分页中断导致误判。
