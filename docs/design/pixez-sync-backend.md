...
5. 单次完整导出成功结束后，将同一用户同一 `restrict` 下“本轮未出现但历史存在且尚未移除”的记录标记为 `removed=true`，并写入 `removed_at`。
6. 若导出中途失败，不执行缺失标记，避免分页中断导致误判。
...
| `restrict` | `TEXT` | NOT NULL, INDEX | 收藏范围，当前为 `public` |
